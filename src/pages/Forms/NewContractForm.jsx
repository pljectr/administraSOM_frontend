        ...
        <Container maxWidth="lg" sx={{ mt: 4 }} className="form-container"> {/* Container maior */}
            <Paper sx={{ p: 4 }} elevation={3} className="main-form-paper">
                ...
                <Grid container spacing={4} sx={{ mb: 4, height: 'fit-content' }} className="main-sections-grid"> {/* Ajustado height para fit-content */}
                    {/* MAPA - 30% */}
                    <Grid item xs={12} md={4} sx={{ display: 'flex' }} className="map-section"> {/* display: flex para o item */}
                        <Paper sx={{ p: 2, flexGrow: 1, display: 'flex', flexDirection: 'column' }} elevation={1} className="map-card"> {/* flexGrow: 1 para o Paper */}
                            <Typography variant="h6" gutterBottom className="section-title">
                                Localização das OMs
                            </Typography>
                            {/* O Box map-canvas agora sem centralização para o mapa ocupar todo o espaço */}
                            <Box sx={{ flexGrow: 1, minHeight: 300, display: 'flex' }} className="map-canvas"> 
                    ...
                    <Grid item xs={12} md={8} sx={{ display: 'flex' }} className="general-info-section"> {/* display: flex para o item */}
                        <Paper sx={{ p: 3, flexGrow: 1 }} elevation={1} className="general-info-card"> {/* flexGrow: 1 para o Paper */}
                            ...
                            <Grid container spacing={2}>
                                ...
                                <Grid item xs={12}>
                                    <TextField label="Nome do Contrato" fullWidth value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} />
                                </Grid>
                                <Grid item xs={12}>
                                ...
                                <Grid item xs={12}>
                                    <TextField label="Descrição Detalhada" fullWidth multiline rows={3} value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} />
                                </Grid>
                                <Grid item xs={12} sm={6}>
                                    <TextField
                                        label="Data Expiração Administrativa"
                                        type="date"
                                        fullWidth
                ...
                <Paper sx={{ p: 3, mb: 4 }} elevation={1} className="schedule-section form-card">
                    ...
                    {scheduleItems.map((item, index) => (
                        <Grid container spacing={2} key={index} sx={{ mb: 2, alignItems: 'center' }} className="schedule-item-row">
                            <Grid item xs={12} sm={4}>
                                <TextField
                                    label="Mês/Ano"
                                    type="month"
